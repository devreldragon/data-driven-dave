...
def showTitleScreen(screen, tileset, ui_tiles):
    ...
    # create a random exception on purpose
    xCoord = random.randint(0, 9)
    yCoord = random.randint(0, 9)
    yCoord = 0
    # exceptionCalc = xCoord / yCoord
...
def showInterpic(completed_levels, screen, GamePlayer, tileset, ui_tileset):
    ...
    # init player
    (player_absolute_x, player_absolute_y) = interpic_level.initPlayerPositions(
        0, GamePlayer)
    ...
    # init messages
    intertext = "GOOD WORK! ONLY " + \
        str(NUM_OF_LEVELS - completed_levels + 1) + " MORE TO GO!"
    ...
    while not player_reached_boundary:
        ...
        screen.printPlayer(GamePlayer, player_absolute_x,
                           player_absolute_y, tileset)
        ...
        player_reached_boundary = (
            player_absolute_x >= screen.getUnscaledWidth())
...
def showWarpZone(completed_levels, screen, GamePlayer, tileset, ui_tileset):
    ...
    while not player_reached_bottom:
        ...
        player_reached_bottom = (
            player_absolute_y >= screen.getUnscaledHeight())
...
def main():
    ...
    while game_open:
        ...
        # Available Keys
        movement_keys = [pygame.K_UP, pygame.K_LEFT,
                         pygame.K_RIGHT, pygame.K_DOWN]
        inv_keys = [pygame.K_LCTRL, pygame.K_RCTRL,
        ...
        while not ended_game:
            ...
            (player_position_x, player_position_y) = Level.initPlayerPositions(
                current_spawner_id, GamePlayer)

            spawner_pos_x = Level.getPlayerSpawnerPosition(current_spawner_id)[
                0]
            ...
            while not ended_level:
                    ...
                    # hit a key
                        ...
                        # use something from the inventory
                            ...
                            if GamePlayer.inventoryInput(inv_keys.index(event.key)) and not friendly_shot:
                                friendly_shot = Level.spawnFriendlyFire(
                                    GamePlayer.getSpriteDirection())
                                friendly_shot_x, friendly_shot_y = player_position_x + \
                                    GamePlayer.getDirectionX().value * WIDTH_OF_MAP_NODE, player_position_y
                ...
                player_close_to_right_boundary = (player_position_x >= game_screen.getXPositionInPixelsUnscaled(
                ) + game_screen.getUnscaledWidth() - BOUNDARY_DISTANCE_TRIGGER)
                reached_level_left_boundary = (game_screen.getXPosition() <= 0)
                reached_level_right_boundary = (game_screen.getXPosition(
                ) + game_screen.getWidthInTiles() > Level.getWidth())

                # move screen left
                if player_close_to_left_boundary and not reached_level_left_boundary:
                    game_screen.moveScreenX(
                        Level, -15, tileset, ui_tileset, GamePlayer, current_level_number)
                # move screen right
                ...
                # not moving (just update the screen)
                    ...
                    if GamePlayer.getCurrentState() != STATE.DESTROY:
                        ...
                        game_screen.printPlayer(
                            GamePlayer, player_position_x - game_screen.getXPositionInPixelsUnscaled(), player_position_y, tileset)
                    elif not ended_game:
                ...
                game_screen.printUi(ui_tileset, GamePlayer,
                ...
                if not ended_level:
                    ...
                    if GamePlayer.inventory["jetpack"] == 1 or jetpack_ui:
                        game_screen.updateUiJetpack(
                            ui_tileset, GamePlayer.inventory["jetpack"])
                        jetpack_ui = True
            ...
            if current_level_number > NUM_OF_LEVELS and ended_level and not ended_game:
                ...
                option = showWarpZone(
                    current_level_number, game_screen, GamePlayer, tileset, ui_tileset)
                ended_game = option
                game_open = not option
